...
>
> *Cập nhật 09-13, sau các bảng dưới:* **SÀN ĐÃ BỎ 2026-09-13 (nạp, FC thử trên target)** — xem 11.1 #12a. Các ca "dưới sàn" bên dưới là
> **lịch sử**. Firmware hiện tại, nằm bàn: "xuống 0,3" → `OB_T_UP = −0,300`, Δ`CLP` = 0; "xuống 0,5" →
> `−0,300`, Δ`CLP` tăng (xuống chậm sát đất, 9.6).
>
...
| `ATTITUDE` | 30 | **30 Hz** *(1.5)* | 50.0 *(bản ≤ 1.4)* | `/mavros/imu/data` |
| ~~`LOCAL_POSITION_NED`~~ | 32 | **ngừng từ 1.5** — thay bằng `ODOMETRY` | 30.3 *(bản ≤ 1.4)* | ~~`/mavros/local_position/pose`~~ — plugin đã tắt |
| `GLOBAL_POSITION_INT` | 33 | **1 Hz** *(1.5)* | 10.0 *(bản ≤ 1.4)* | `/mavros/global_position/rel_alt`, `.../global` |
| ~~`VFR_HUD`~~ | 74 | **ngừng từ 1.5** | 10.0 *(bản ≤ 1.4)* | ~~`/mavros/vfr_hud`~~ — plugin đã tắt |
| `HIGHRES_IMU` | 105 | **30 Hz** *(1.5)* | 50.0 *(bản ≤ 1.4)* | `/mavros/imu/data_raw`, `/mavros/imu/mag` |
...
| `SET_POSITION_TARGET_LOCAL_NED` | 84 | setpoint vận tốc — mục 5.2 | [CHỐT] — Pi đo đầu-cuối 09-13/14: kiểm dấu 3.4 bốn trục, bao 9.6, `position_controller_node` 20 Hz → `OB_RX_OK` +20/s, FC vào OFFBOARD (`OB_STATE = 2`). **Trước 09-13 FC loại mọi khung `0x07C7`** — mục 5.2 |
...
| ~~`LANDING_TARGET`~~ | 149 | **không xử lý** | **[PHẾ BỎ từ 2026-09-14 — 11.1 #13c]** — kiến trúc 11.1 #12: Pi đóng vòng vận tốc theo marker. Chưa từng hiện thực nên không có gì ngừng trên dây → không tăng số (ngoại lệ 10.4). ID 149 không tái sử dụng |
...
Sau đó Pi **tự do arm và disarm** không cần thêm thao tác nào từ tay cầm — **trừ cổng độ cao của
DISARM từ hợp đồng 1.3:** DISARM thường chỉ được nhận khi máy bay ≤ 20 cm trên mặt đất; cao hơn trả
`TEMPORARILY_REJECTED`; `param2 = 21196` cắt ở mọi độ cao (11.1 #12d–f). Người lái cắt bằng ch5 không
qua cổng này.
...
>
> **Ga lúc Pi arm (FC 09-14, 11.1 #13):** vòng giữ độ cao vào chế độ với tích phân nạp theo ga đang
> giữ, nên ngay lúc arm động cơ lên **~50 % ga** và giữ ở đó khi Pi lệnh `vz = 0`. Ga treo
> `althold_hover_thr = 0,65` là **số chủ dự án đã đo**; ga treo thật cao hơn một chút — cố ý làm tròn
> xuống để máy bay không nhấp nhô vì hiệu ứng mặt đất. 50 % < 65 % → **máy bay nằm yên trên đất sau khi
> arm**, chỉ rời đất khi Pi ra lệnh leo. Kiểm lại ở 12.B khi có cánh.
...
**[CHỐT — Pi đo 09-14: `OB_ARM_RDY = 1` → ARM `ACCEPTED`, 12.A4] Kênh sẵn sàng arm — thêm vào nhóm `NAMED_VALUE_INT` 2 Hz:**
...
| 84 | `SET_POSITION_TARGET_LOCAL_NED` | Pi→FC | [CHỐT] — Pi đo đầu-cuối 09-14 (3.4, 5.1) |
...
| 105 | `HIGHRES_IMU` | FC→Pi | [CHỐT] — **30 Hz từ 1.5** (11.1 #14) |
| 106 | `OPTICAL_FLOW_RAD` | FC→Pi | [ĐỀ XUẤT] — cần đo hệ số quy đổi radian trước |
| 132 | `DISTANCE_SENSOR` | FC→Pi | [CHỐT] — Pi đã bật plugin và đo 09-13 (mục 4.1). `min_distance = 15` cm từ 09-13 (11.2) — *Pi đo lại 09-14: `min_distance = 15` trên dây, `Range.min_range = 0.15` qua MAVROS* |
...
| 149 | ~~`LANDING_TARGET`~~ | Pi→FC | **[PHẾ BỎ từ 2026-09-14 — 11.1 #13c]** — FC chưa từng xử lý; Pi tắt plugin `landing_target` (P1). Số 149 không tái sử dụng (10.4 bước 4) |
...
| 253 | `STATUSTEXT` | FC→Pi | [THOẢ THUẬN] — FC đã hiện thực 09-13, **Pi chưa đo** (P12, không chặn FC) |
...
| `OB_ARM_RDY` | INT | FC | 1 = đang chờ Pi arm, mọi điều kiện thoả (6.3) | [CHỐT] — Pi đo 09-14: `1` khi ch8, ch5 lên, ch6 POSHOLD → ARM `ACCEPTED`; `0` sau khi arm |
| `OB_ARM_BLK` | INT | FC | bitmask lý do chưa sẵn sàng arm (6.3) — bảng bit ngay dưới | [CHỐT] — Pi thấy `0x0480`, `0x0082`, `0x0002`, `0x0000` (09-13/14), khớp bảng; chỉ để hiển thị |
...
| `offboard_max_alt_m` | **5,0** m | **trần độ cao**, chỉ áp khi `altitude_valid`: trên trần chỉ được xuống; lệnh lên → `climb = 0`, đếm `OB_RX_CLP`, không vào `KEP_DAI` |
| `offboard_min_alt_m` | *(0,3)* | **KHÔNG CÒN TÁC DỤNG từ 2026-09-13** — sàn độ cao đã bỏ (11.1 #12a). Tham số giữ trong bảng chỉ để không mất cấu hình đã lưu; firmware không đọc |
| Cổng DISARM từ Pi *(1.3)* | mốc mặt đất **0,17 m** (hằng số) + **0,20 m** → nhận DISARM thường khi độ cao ước lượng ≤ **0,37 m**; mã ép `21196` | xem 11.1 #12d–f. Hằng số firmware, không phải tham số |
| Xuống chậm sát đất | **0,3 m/s** khi độ cao ước lượng ≤ **1,2 m** (laser; ≈ 1 m trên mặt đất) — hằng số firmware | lệnh xuống nhanh hơn bị kẹp về −0,3 m/s, đếm `OB_RX_CLP`, **không** vào `KEP_DAI`. Pi hạ cánh bằng setpoint thường (11.1 #12). Chỉ áp khi `altitude_valid` |
...
1. **`flight_custom_version`** (8 byte git hash) trong `AUTOPILOT_VERSION` — chính xác
   tuyệt đối, biết đúng bản build nào **nếu build từ commit sạch** (mục 9.5). [CHỐT — Pi đo 09-13; mọi bản đã nạp tới 1.3 đều `FC_DIRTY = 1`, 11.1 #13b]
2. **`NAMED_VALUE_INT` `FC_CTR_VER`** = `MAJOR × 10000 + MINOR × 100`. Hợp đồng 1.1 →
   `10100`, 1.2 → `10200`, 1.3 → `10300`, 1.4 → `10400`, 1.5 → `10500`. Phát 2 Hz cùng nhóm `OB_*`. [CHỐT — Pi đo `10100`, `10200`, `10300` trên dây và MAVROS; tên đổi từ `CONTRACT`,
   mục 9.3]
...
### 11.0 Tổng hợp trạng thái — Pi, 2026-09-14, hợp đồng 1.3 (`FC_CTR_VER = 10300`)

> **Cập nhật FC 09-14 — hợp đồng 1.4 (`FC_CTR_VER = 10400`):** cả ba đề xuất 11.1 #13 đã làm. (a) cờ
...
| Nhóm | Mục | Ở đâu |
|---|---|---|
| **[CHỐT] hôm nay** | `SET_POSITION_TARGET_LOCAL_NED` đầu-cuối: dấu 4 trục, bao 9.6, node Pi 20 Hz, FC vào OFFBOARD | 3.4, 5.1, 9.1 |
| **[CHỐT] hôm nay** | `OB_ARM_RDY`, `OB_ARM_BLK`, `OB_DIS_RDY`; ARM/DISARM thật qua `fc_command_bridge_node` | 6.3, 9.3, 12.A4 |
| **[CHỐT] hôm nay** | Cổng DISARM 20 cm cả hai phía + `21196` | 11.1 #12, 12.A4 |
| **[CHỐT] hôm nay** | `COMMAND_ACK` qua MAVROS — bắt buộc `confirmation = 1` | 7 |
| **[CHỐT] hôm nay** | `FC_CTR_VER`, `flight_custom_version` | 10.1 |
| **Đóng** | 11.1 #2, #3, #5, #7, #8, #9, #12 | 11.1 |
| **Đề xuất cho FC** | **11.1 #13** — (a) khai cờ capability `0x0080`; (b) nạp bản sạch `FC_DIRTY = 0` trước 12.B; (c) bỏ `LANDING_TARGET` (149). **Ngoài #13, Pi không còn yêu cầu nào với FC.** *FC làm xong cả ba 09-14 — hợp đồng 1.4* | 11.1 #13 |
| Việc đo còn lại **phía Pi** — không chặn FC | `STATUSTEXT`; thử dấu `ODOMETRY` bằng tay; `OB_ARM_BLK` từng bit; xuống chậm khi laser > 1,2 m; 12.A1 (`relative_alt`, bit flow, `CMODE`); 12.A4 (`OB_AUTH` khi chạm cần, ch8 xuống) | P12, 12.A |
| Chờ **điện động cơ** (12.B) | chuỗi `KEP_DAI`, hết hạn setpoint, ngưỡng chạm đất khi cánh quay, covariance IMU khi rung, điều kiện từ kế 10.6a | 12.B, 10.6a |
| **Để sau**, chưa ai làm | `OPTICAL_FLOW_RAD` (106), `VIBRATION` (241) | 9.1 |
...
>
> **Đã làm từ hợp đồng 1.3 (nạp 09-14, `FC_CTR_VER = 10300`) — Pi đồng ý 09-13, không MAJOR:**
>
> | # | Việc | Trạng thái |
> |---|---|---|
> | 4 | **Cổng DISARM theo độ cao:** DISARM từ Pi chỉ nhận khi ≤ **20 cm** trên mặt đất (laser ≤ ~37 cm); cao hơn → `TEMPORARILY_REJECTED`; `param2 = 21196` cắt ở mọi độ cao. Người lái ch5 không đổi | **ĐÃ LÀM.** Pi xác nhận chưa node nào disarm trên không → không MAJOR. Mốc mặt đất đổi thành **hằng số 0,17 m** (trả lời Pi, 11.1 #12d) |
> | 5 | `NAMED_VALUE_INT` **`OB_DIS_RDY`** | **ĐÃ LÀM**, theo nghĩa Pi đề nghị: 0 khi chưa arm hoặc mất quyền |
> | 6 | **Pi tự nhận ra chạm đất** — gợi ý: laser ≈ 0,17 m + `vz` ≈ 0 dù đang lệnh xuống, giữ ~1 s. **Không dùng "ga thấp"**: chạm đất ga vẫn ~27 %, không bao giờ < 10 % | Pi chốt 09-13; FC góp ý mốc mặt đất (11.1 #12e) |
>
...
| **2** | Chạy quy trình kiểm dấu (mục 3.4) | **Pi** | mọi lệnh điều khiển; đóng mục 3.1–3.2 bằng phép đo | **XONG 09-14** — bốn trục đúng dấu qua `OB_T_*` (3.4); bao 9.6 đúng sau #11; đầu-cuối qua `position_controller_node` |
| **3** | `fields_updated` vẫn bật cờ từ kế | **FC** | tin cậy `HIGHRES_IMU.mag`; cổng mở `FRAME_LOCAL_NED` | **XONG 09-13** — đúng thiết kế; điều kiện thật ở 10.6a |
| ~~4~~ | ~~Launch MAVROS crash~~ | Pi | — | **XONG 2026-09-13** — xem mục 8.3 |
| **5** | Đường arm đi qua cổng nào? | **FC** | nghiệm thu 12.B | **XONG 09-14** — chỉ UART8; Pi đã arm/disarm qua UART8 (12.A4) |
...
| **7** | Ba mâu thuẫn + hai điểm nhỏ Pi báo | **FC** | — | **XONG 09-13** — đã sửa câu chữ, xem chi tiết |
| **8** | Timeout heartbeat Pi 3000 ms không được dùng | **FC** → hai bên chốt | hiểu đúng lưới an toàn | **XONG 09-14** — hai bên đồng ý **không** thêm hành động khi mất heartbeat; giám sát nội bộ là nợ Pi P6 |
| **9** | Thứ tự byte `flight_custom_version` | **FC** | đọc đúng bản build từ log MAVROS | **XONG 09-13** — `uint64` little-endian từ 1.2, Pi đo MAVROS in đúng chiều |
...
[CHỐT — hai bên đồng ý 09-13, đóng 09-14] **Không thêm** hành động khi mất heartbeat: timeout setpoint đã chặt hơn cho
...
- **(a)** Kẹp tốc độ xuống **không** cố ý bằng 0 — cận là `−1,0 m/s`. **Sàn 0,3 m là cố ý:** Pi
  ...
  *Cập nhật 09-13:* chủ dự án quyết định bỏ sàn — **SÀN ĐÃ BỎ 2026-09-13 (nạp, FC thử trên target)** — xem 11.1 #12a. Pi hạ cánh bằng setpoint.
...
**Chi tiết #12 — đề xuất hạ cánh chính xác (FC 09-13)** — *[CHỐT: FC làm ở 1.3, Pi đo 09-14; lịch sử trao đổi giữ bên dưới].*

Chủ dự án chốt hướng 09-13: **Pi hạ cánh bằng cách đọc marker.** Các quyết định đã chốt phía chủ dự án:
(1) ~~Pi báo bắt đầu hạ bằng `MAV_CMD_NAV_LAND`~~ — **bỏ 09-13**: chủ dự án bỏ luôn sàn độ cao, nên
không cần lệnh nào để "mở sàn"; (2) **Pi** tự nhận ra chạm đất và gửi DISARM — FC
...
chọn cổng theo độ cao cho đơn giản; khung chịu được rơi từ 20 cm.* Phần (a) đã làm theo lệnh chủ dự
án; phần (d)–(f) là cách điền FC đề xuất — Pi sửa/đồng ý **trước** khi FC viết code.

**Kiến trúc — đóng luôn nợ P1:** Pi đóng vòng ngang (marker → `vx`/`vy`/`yaw_rate`) và chọn tốc độ
xuống; FC giữ vòng độ cao, kẹp tốc độ xuống sát đất, và canh cửa DISARM. FC **không** xử lý
`LANDING_TARGET` (149) — bỏ khỏi 9.1 khi chốt. **Không** có chế độ LAND, **không** có pha hạ cánh,
**không** có lệnh mới: với FC, hạ cánh chỉ là OFFBOARD bình thường với `vz` hướng xuống.

**a) Bỏ sàn độ cao — ĐÃ LÀM, nạp 2026-09-13.**

Sàn `offboard_min_alt_m = 0,3 m` bỏ hẳn. Lý do: laser nằm đất đã đọc ~0,17 m nên sàn chỉ cao hơn mặt
đất ~13 cm — bảo vệ rất ít; nó chặn đúng việc Pi cần làm (hạ cánh); và nó sinh ra lỗi 11.1 #11. Tham
số giữ trong bảng nhưng không còn tác dụng (9.6).

Thay bằng **xuống chậm sát đất**: độ cao ước lượng ≤ **1,2 m** (≈ 1 m trên mặt đất) thì tốc độ xuống
bị kẹp về **0,3 m/s** — mọi lần chạm đất trong OFFBOARD đều nhẹ, dù Pi hạ có chủ đích hay Pi lỗi. Kẹp
này là kẹp bao: đếm `OB_RX_CLP`, **không** vào chuỗi `KEP_DAI` — Pi gửi cố định −0,5 m/s suốt lúc hạ
cũng không mất quyền. Trần 5 m giữ nguyên (kèm dải chết 0,01 m/s của #11).

**FC thử trên target 09-13** (GDB qua SWD như #11, cấu hình đã lưu còn nguyên sau khi nạp):

| Độ cao giả lập | Lệnh | `OB_T_UP` | Δ`CLP` | chuỗi `KEP_DAI` |
|---|---|---|---|---|
| 0,18 (nằm đất) | xuống 0,3 | −0,300 | 0 | 0 |
| 0,18 | xuống 0,5 | −0,300 | +1 | 0 |
| 0,18 | xuống 10 | −0,300 | +1 | 1 (kẹp tốc độ −1,0 trước) |
| 0,18 | trái 0,5, `vz` = +6e-17 | 0 | 0 | 0 |
| 0,18 | lên 0,3 | +0,300 | 0 | 0 |
| 1,19 | xuống 0,8 | −0,300 | +1 | 0 |
| 1,21 | xuống 0,8 | −0,800 | 0 | 0 |
| 5,0 (trần) | lên 0,3 | 0 | +1 | 0 |
| 5,0 | phải 0,5, `vz` = −6e-17 | 0 | 0 | 0 |
| 5,0 | xuống 0,5 | −0,500 | 0 | 0 |

**Hệ quả Pi cần biết:**

- Pi **hạ cánh bằng `SET_POSITION_TARGET_LOCAL_NED` như 5.2**, không cần lệnh gì thêm. Nên tự kẹp
  ≤ 0,3 m/s dưới ~1,2 m laser (P9) để không chạm `OB_RX_CLP`.
- **Pi lỗi ra lệnh xuống giờ đưa máy bay chạm đất** (nhẹ, 0,3 m/s) và nằm đó với cánh quay ~27 % ga,
  OFFBOARD vẫn chạy. Trước đây sàn giữ máy bay lơ lửng ~13 cm. Người lái cắt bằng ch5, hoặc Pi disarm.
- **Đừng lơ lửng lâu dưới ~0,25 m laser:** flow bị loại dưới 0,20 m (h), kênh ngang không giữ chỗ.
...
+ 0,20 m**. *Mốc mặt đất* = **hằng số 0,17 m** — số laser đọc khi máy bay nằm đất, do cách lắp (11.2
`DISTANCE_SENSOR`). Tức cổng mở khi độ cao ước lượng **≤ 0,37 m**. Không lọc thời gian: gần đất là mở
ngay. Ngưỡng là hằng số firmware (`LINK_DISARM_GROUND_M`, `LINK_DISARM_MAX_HEIGHT_M`), không phải tham số.

**FC trả lời hai câu Pi hỏi 09-13:**

1. **Arm lúc cầm tay / đứng trên bục → mốc lệch.** Đúng với bản đề xuất (mốc lấy lúc arm). **FC đã đổi
   sang mốc hằng số 0,17 m** — không phụ thuộc lúc arm, cầm tay arm không làm lệch gì. Trên bục: laser
   đo tới mặt bục, nên cổng tính theo **mặt ngay dưới máy bay** — đúng với độ cao rơi thật. Điều ngược
   lại cần biết: bay ngang qua mép bục/hộp cao ≥ ~20 cm, laser đọc tới mặt hộp → cổng **mở** dù sàn nhà
   còn xa. Chấp nhận: cổng là lưới an toàn cho lỗi Pi, không phải điều kiện Pi dựa vào.
   **Góp ý tiêu chí (e) của Pi** — "laser ≤ mốc đo lúc arm + 0,05 m" vướng đúng lỗi này. Đề nghị Pi dùng
   mốc **0,17 m** như FC, hoặc lấy số laser **khi đang disarm nằm đất trước lúc cất cánh** (không phải
   lúc arm, vì arm có thể cầm tay).
2. **Nghiêng > 25° → laser bị loại → cổng đóng?** **Không tự đóng.** Cổng dùng độ cao **ước lượng**
   (EKF), không dùng laser thô. Khi laser bị loại, EKF vẫn `altitude_valid` (một khi đã hợp lệ thì giữ)
   và chạy tiếp bằng baro — con số giữ gần giá trị cuối rồi **trôi theo baro** (cỡ vài dm sau vài phút).
   Máy bay lật nằm đất ngay sau va chạm: độ cao ước lượng còn gần 0,2 m → cổng **thường mở**. Nhưng không
   bảo đảm: baro trôi có thể đẩy qua 0,37 m → cổng đóng. Đường cắt chắc chắn khi lật vẫn là **ch5**.
   Theo cam kết của Pi (không tự gọi 21196), đó là đúng thiết kế.
...
**Báo lên dây:** `NAMED_VALUE_INT` **`OB_DIS_RDY`** trong nhóm 2 Hz — **một nghĩa duy nhất, theo đề nghị
Pi:** 1 = đã arm **và** Pi có quyền **và** gần đất — DISARM thường lúc này sẽ `ACCEPTED` và cắt động cơ;
0 = mọi trường hợp khác, **kể cả chưa arm** (dù DISARM lúc chưa arm vẫn trả `ACCEPTED` vì không có gì để
cắt — 6.2). Cùng cách hiểu `OB_ARM_RDY`: FC tính bằng đúng hàm kiểm lệnh. `landed_state`
...
Người lái cắt bằng ch5 **không đổi** — cổng này chỉ áp cho lệnh từ Pi, ở mọi lúc (hạ cánh hay bay
thường).
...
**g)** ~~`OB_LAND`, `STATUSTEXT` pha hạ cánh~~ — **bỏ 09-13** cùng pha `HA_CANH`.
...
Phiên bản: **hợp đồng 1.3** (`FC_CTR_VER = 10300`, nạp 09-14) — thêm `OB_DIS_RDY` và cổng DISARM, gộp cả
bỏ sàn + xuống chậm (a). Cổng DISARM đổi hành vi một lệnh đang dùng, nhưng **Pi xác nhận 09-13 chưa node
nào gửi DISARM khi đang bay** (`fc_command_bridge_node.on_arm` còn TODO, `failsafe_monitor_node` không
disarm) → **không MAJOR**, theo đề nghị Pi.

**Cam kết phía Pi (ghi lại 09-13):** tách hai lệnh — DISARM thường khi hạ cánh, và cắt khẩn cấp `21196`
riêng, **chỉ khi người ra lệnh qua GCS**, không node nào tự gọi. DISARM bị `TEMPORARILY_REJECTED` → thử
lại trong 3 s rồi báo người lái cắt bằng ch5, **không** tự chuyển sang `21196`.

**FC thử trên target 09-14** (GDB qua SWD, giả lập trạng thái đã arm và ch8 lên trong lúc lõi dừng —
**không** arm thật; board reset sau khi thử):

| # | Trạng thái giả lập | Gọi | Kết quả | Kỳ vọng |
|---|---|---|---|---|
| D0 | chưa arm | `OB_DIS_RDY` | 0 | 0 |
| D1 | chưa arm | DISARM thường | `ACCEPTED` | `ACCEPTED` |
| D2 | arm, có quyền, 1,00 m | `OB_DIS_RDY` | 0 | 0 |
| D3 | arm, có quyền, 1,00 m | DISARM thường | `TEMPORARILY_REJECTED` | `TEMPORARILY_REJECTED` |
| D4 | arm, có quyền, 0,38 m | `OB_DIS_RDY` | 0 | 0 (ngưỡng 0,37) |
| D5 | arm, có quyền, 0,36 m | `OB_DIS_RDY` | 1 | 1 |
| D6 | như D5, `altitude_valid` sai | `OB_DIS_RDY` | 0 | 0 |
| D7 | arm, `KHOA` (mất quyền), 0,36 m | `OB_DIS_RDY` | 0 | 0 |
| D8 | arm, `KHOA`, 1,00 m | DISARM + `21196` | `DENIED` | `DENIED` — mã ép không vượt quyền |
| D9 | arm, có quyền, 1,00 m | DISARM + `21196` | `ACCEPTED`, về `SAFE` | `ACCEPTED` |
| D10 | arm, có quyền, 0,20 m | DISARM thường | `ACCEPTED`, về `SAFE` | `ACCEPTED` |

Sau reset, trên dây: `FC_CTR_VER = 10300`, `OB_DIS_RDY = 0` (chưa arm), bộ đếm rớt khung TX = 0.

**Chi tiết #12 — Pi trả lời 09-14.** *(Viết trước khi FC làm 1.3; FC đã áp dụng ở trên. Phần Pi nhận trả
lời của FC nằm cuối mục này.)*

**Pi đo phần (a) đã nạp, 09-14** — drone nằm bàn (laser 0,18–0,20 m, `ODOMETRY z` 0,186, phương sai
z 8,9e-5), chưa arm, qua `/mavros/setpoint_raw/local` như 3.4:

| Lệnh (FLU) | `OB_T_UP` / `RGT` | Δ`CLP` | Kỳ vọng FC |
|---|---|---|---|
| xuống 0,3 | −0,300 | 0 | đạt |
| xuống 0,29 | −0,290 | 0 | đạt |
| xuống 0,5 | −0,300 | +37 | đạt |
| xuống 10 | −0,300 | +37 | đạt |
| trái 0,5 / phải 0,5 | `RGT` −0,500 / +0,500 | 0 | đạt |
| lên 0,3 | +0,300 | 0 | đạt |

`DISTANCE_SENSOR.min_distance = 15`, `current_distance` 18–20, `signal_quality = 100`; ROS
`/mavros/mtf01p` `min_range = 0.15`, `range = 0.18` — khớp. Chưa đo: ca kê cao > 1,2 m ("xuống 0,8"
→ `−0,800`), cần người kê drone.

**(d), (f) Cổng DISARM 20 cm + mã ép `21196`: Pi đồng ý.**

- **Node nào đang dùng DISARM trên không: KHÔNG CÓ.** Pi rà toàn bộ `src/` 09-14:
  - Đường duy nhất tới `/mavros/cmd/arming` là `fc_command_bridge_node.on_arm` (service `Arm`,
    `arm = false` là disarm) — **còn là TODO, trả `success = false`, chưa gửi gì lên dây**.
  - `mission_manager_node` có client tới `~/arm` nhưng chưa gọi disarm ở đâu.
  - `failsafe_monitor_node` leo thang bằng `FcSimpleCommand` (`LAND_NOW`/hạ cánh khẩn cấp), **không**
    disarm; hàm đó cũng là TODO.
  - Hằng `FC_CMD_DISARM` trong `FcCommand.msg` không có ai dùng.

  → Không có hành vi đang chạy nào bị đổi, **Pi đề nghị không tăng MAJOR**: gộp cổng DISARM với
  `OB_DIS_RDY` vào **MINOR 1.3**, ghi rõ ngoại lệ như lần #9.
- **Pi cam kết khi viết code:** `fc_command_bridge_node` tách hai lệnh — DISARM thường (không
  `param2`) cho hạ cánh, và cắt khẩn cấp (`param2 = 21196`) là lệnh riêng, **không** node nào gọi tự
  động; chỉ gọi khi có lệnh khẩn cấp rõ ràng từ người (GCS). `failsafe_monitor_node` không bao giờ
  dùng `21196`.
- **Khi DISARM bị `TEMPORARILY_REJECTED` dù Pi thấy đã chạm đất** (vd độ cao không hợp lệ): Pi gửi lại
  2 Hz tối đa 3 s, rồi báo người lái cắt bằng ch5. Pi **không** tự leo thang lên `21196`. `DENIED` →
  dừng ngay (6.2).
- Hai câu hỏi cho FC khi viết code: (1) arm **lúc đang cầm tay / trên bục** thì mốc mặt đất lệch —
  FC có kẹp mốc vào khoảng hợp lý (vd ≤ 0,5 m) không? (2) Cổng dùng độ cao **đã bù nghiêng** — nghiêng
  > 25° laser bị loại (9.6) thì cổng đóng; xác nhận đó là ý định.

**(d) Tên `OB_DIS_RDY`: Pi đồng ý tên** (10 ký tự, đúng tiền tố `OB_`, 9.3). Một đề nghị về nghĩa, cho
khớp `OB_ARM_RDY`: **`OB_DIS_RDY = 0` khi đang không arm và khi Pi không có quyền** — tức 1 chỉ khi
đã arm, có quyền, và (gần đất theo (d)). Lệnh DISARM lúc chưa arm vẫn `ACCEPTED` như 6.2, nhưng Pi
không cần cờ báo điều đó. Nếu FC giữ nghĩa "1 = DISARM lúc này `ACCEPTED`" theo đúng bảng (f) thì
ghi rõ trạng thái chưa arm trả 1 — Pi dùng được cả hai, chỉ cần ghi một nghĩa. Thêm tên này thì
nhóm 2 Hz có 11 tên dồn một cụm: Pi đặt subscriber depth ≥ 20 (P8).

**(e) Tiêu chí chạm đất — Pi chốt** (không dùng ga, không dùng `landed_state`):

| # | Điều kiện | Nguồn | Ngưỡng |
|---|---|---|---|
| 1 | Laser gần mốc mặt đất | `/mavros/mtf01p` `range` | ≤ **mốc + 0,05 m**; mốc: xem sửa 09-14 ngay dưới |
| 2 | Không còn đi xuống | `/mavros/odometry/in` `twist.linear.z`, covariance vz ≠ `1e6` | \|vz\| < **0,05 m/s** |
| 3 | Pi vẫn đang ra lệnh xuống | setpoint của `position_controller_node` | `vz` ≤ −0,1 m/s |
| 4 | Giữ liên tục | 1–3 cùng đúng | **1,0 s**; một mẫu sai là đếm lại |
| 5 | FC mở cổng | `OB_DIS_RDY`, `OB_AUTH` | cả hai = 1 |

Đủ 1–5 → gửi DISARM thường (không `param2`). Ngưỡng 1, 2 là **số tạm**: nhiễu tĩnh trên bàn đo được
laser 1–3 cm, `vz` ≈ 0,003 m/s; **chưa đo khi cánh quay** (hiệu ứng mặt đất, rung) — đo lại ở 12.B
rồi chốt.

**Pi nhận trả lời FC 1.3 (09-14):**

- **Mốc mặt đất của tiêu chí (e) — Pi sửa theo góp ý FC.** Mốc = **trung vị `range` trong 2 s lúc máy
  bay đang disarm, nằm yên trước khi cất cánh** (`armed = false`, \|vz\| < 0,05 m/s). Chỉ nhận nếu nằm
  trong 0,15–0,25 m; ngoài khoảng đó hoặc không có mẫu (vd arm lúc cầm tay) → mốc = **0,17 m** như FC.
  Không lấy lúc arm nữa.
- **Bay ngang qua hộp cao:** cổng FC có thể mở — Pi ghi nhận. Tiêu chí (e) của Pi không bị ảnh hưởng vì
  đòi thêm "đang ra lệnh xuống" và \|vz\| ≈ 0 giữ 1 s: bay ngang không thoả điều kiện 3. Hạ xuống và
  bị mặt hộp chặn thì đúng là đã chạm — disarm ở đó là đúng độ cao rơi thật.
- **Nghiêng > 25°, cổng không tự đóng, baro trôi:** Pi ghi nhận. Không đổi cam kết: Pi không tự gọi
  `21196`; lật máy thì người lái cắt ch5.
- `OB_DIS_RDY` theo đúng nghĩa Pi đề nghị — đồng ý. Pi đo 1.3: 12.A4.

**Chi tiết #13 — đề xuất của Pi 09-14, gom cho lần nạp FC tiếp theo [ĐỀ XUẤT].** Không mục nào chặn
bàn test; không cần nạp riêng.

- **(a) Khai cờ `MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED` (`0x0080`)** → `capabilities =
  0x2080`. Theo 9.5 và 10.3 bước 7, cờ chỉ khai sau khi kiểm dấu 3.4 đạt — nay đã đạt và đo đầu-cuối.
  Đây là thứ mới lên dây → theo #10 là **MINOR 1.4** (`FC_CTR_VER = 10400`). Pi không có code dựa vào
  cờ này; Pi sẽ đo lại `AUTOPILOT_VERSION` khi FC nạp.
- **(b) Bản firmware dùng cho 12.B build từ commit sạch (`FC_DIRTY = 0`).** Mọi bản đã nạp từ 1.1 tới 1.3
  đều `FC_DIRTY = 1` và cùng hash `8b9b35f7b2223c3a`; ba lần sửa hành vi (#11, bỏ sàn, 1.3) Pi chỉ phân
  biệt được bằng `FC_CTR_VER` và mốc nạp ghi tay. Đúng quy tắc 9.5 — Pi thêm "`FC_DIRTY = 0` và hash có
  trong lịch sử phiên bản" vào điều kiện bắt đầu 12.B.
- **(c) Bỏ `LANDING_TARGET` (149)** khỏi 5.1 và 9.1 theo 10.4. FC chưa từng xử lý, kiến trúc #12 không dùng
  → không phá gì, không tăng số. Phía Pi: tắt plugin `landing_target` trong `mavros.yaml` và cho
  `landing_target_bridge_node` phát topic nội bộ cho `position_controller_node` (P1).

**Pi chốt:** ngoài (a)–(c), mọi mục đang [CHỐT] giữ nguyên, Pi không yêu cầu FC thay đổi gì thêm. Việc
còn lại là đo phía Pi (P12) và giai đoạn có điện động cơ (12.B). FC đồng ý hoặc sửa (a)–(c) ngay dưới
đây.

**Chi tiết #13 — FC trả lời và làm, 09-14: đồng ý cả ba, hợp đồng 1.4.**

- **(a) Cờ `0x0080`: ĐÃ KHAI.** `capabilities = MAVLINK2 | SET_POSITION_TARGET_LOCAL_NED = 0x2080`. Thứ
  mới lên dây → **MINOR 1.4**, `FC_CTR_VER = 10400` (theo #10).
- **(b) Bản sạch: ĐÃ NẠP.** Firmware commit `6b75e72` (nhánh `mag-qmc5883p-ekf-yaw`, repo FC, commit tại
  máy — chưa đẩy lên remote), build từ cây sạch: `FC_DIRTY = 0`, `flight_custom_version` =
  **`6b75e72b10756804`** (trên dây `04 68 75 10 2b e7 75 6b`, uint64 little-endian). Commit gồm toàn bộ
  thay đổi từ hợp đồng 1.1 tới 1.4. Từ bản này, hash trong `AUTOPILOT_VERSION` đại diện đúng bản đang
  chạy — điều kiện Pi đặt cho 12.B.
- **(c) `LANDING_TARGET` (149): PHẾ BỎ.** FC chưa từng xử lý (bộ nhận chỉ có `HEARTBEAT`, `COMMAND_LONG`,
  `SET_POSITION_TARGET_LOCAL_NED`) → firmware không đổi gì, không có gì ngừng trên dây, **không tăng số**.
  Ghi [PHẾ BỎ] ở 5.1 và 9.1; số 149 không tái sử dụng.
- **Kèm theo — ga lúc arm (FC tự rà trước 12.B):** lúc Pi arm, ga lên ~50 %. Chủ dự án xác nhận ga treo
  `0,65` là số đã đo (thật cao hơn chút, làm tròn xuống để không nhấp nhô vì hiệu ứng mặt đất) → máy bay
  nằm yên sau arm. Ghi ở 6.2 và 12.B.

**FC đo trên dây sau khi nạp 09-14** (giải mã bộ đệm TX UART8 qua SWD; `AUTOPILOT_VERSION` kích bằng gọi
`send_autopilot_version()` qua GDB): `AUTOPILOT_VERSION caps = 0x2080`, `flight_sw = 0x00010000`,
...
**Chi tiết #14 — FC làm, 09-14: đồng ý toàn bộ bảng của Pi, hợp đồng 1.5.**

| Bản tin | Trước | **1.5** |
|---|---|---|
| `ATTITUDE` (30) | 50 Hz | **30 Hz** |
| `HIGHRES_IMU` (105) | 50 Hz | **30 Hz** |
| `GLOBAL_POSITION_INT` (33) | 10 Hz | **1 Hz** |
| `LOCAL_POSITION_NED` (32) | 30 Hz | **ngừng** |
| `VFR_HUD` (74) | 10 Hz | **ngừng** — chọn ngừng hẳn, không để 1 Hz: GCS nhận qua Pi, không qua bản tin này |

Giữ nguyên mọi bản tin khác đúng như Pi liệt kê.

- **Phiên bản: MINOR 1.5**, `FC_CTR_VER = 10500`. Ngừng `LOCAL_POSITION_NED` là bỏ một bản tin đã [CHỐT] —
  theo 10.4 bước 3 là MAJOR; FC **chấp nhận ngoại lệ** Pi nêu, cùng lý do #9: bước 1–2 của 10.4 đã xong
  (`ODOMETRY` song song từ 1.2, Pi chuyển sang và đo), Pi xác nhận không code nào dựa vào, plugin đã tắt.
  Ghi rõ đây là ngoại lệ, không phải tiền lệ. Số 32, 74 không tái sử dụng.
- **Bản sạch:** firmware commit `df169c5`, `FC_DIRTY = 0`, hash `df169c5c0ae19ac5`. Thay bản 1.4
  làm bản đề nghị cho 12.B.
- **Bit flow của `SYS_STATUS`** không đổi nghĩa; trước kia nó "đi kèm" `LOCAL_POSITION_NED`, nay chỉ còn
  đi cùng covariance `1e6` của `ODOMETRY` — cùng một biến, cùng khung.

**FC đo trên dây sau khi nạp 09-14** (60 ảnh chụp bộ đệm TX UART8 qua SWD, kiểm CRC; chu kỳ tính từ mốc
thời gian bên trong khung — `time_boot_ms` / `time_usec` — không phụ thuộc nhịp chụp):

| Bản tin | Chu kỳ đo | Tần số |
|---|---|---|
| `ATTITUDE` (30) | 33,0 ms | **30,3 Hz** |
| `HIGHRES_IMU` (105) | 33,0 ms | **30,3 Hz** |
| `ODOMETRY` (331) | 33,0 ms | 30,3 Hz |
| `DISTANCE_SENSOR` (132) | 50,0 ms | 20 Hz |
| `RC_CHANNELS` (65) | 200 ms | 5 Hz |
| `GLOBAL_POSITION_INT` (33) | mốc 34363 / 37363 / 41363 ms — cách đúng bội 1000 | **1 Hz** |
| `NAMED_VALUE_INT` / `_FLOAT` | 500 ms | 2 Hz |
| `LOCAL_POSITION_NED` (32), `VFR_HUD` (74) | **0 khung** trong 60 ảnh chụp | **đã ngừng** |

`FC_CTR_VER = 10500`, `FC_DIRTY = 0`; `AUTOPILOT_VERSION` (kích qua GDB): `capabilities = 0x2080`,
...
| 2 | `AUTOPILOT_VERSION` đầy đủ + `REQUEST_MESSAGE` (512) | FC | phiên bản hoá hợp đồng (10.1) — **XONG, Pi nghiệm thu 09-13**, thứ tự byte hash xong (11.1 #9); **cờ capability `0x0080` → đề xuất 11.1 #13a** |
...
- **FC im lặng trên UART — gặp 1 lần (Pi 09-14). ĐÃ RÕ: chủ dự án ngắt nguồn FC lúc đó** (xác nhận
  09-14) — không phải lỗi firmware. Sau khi Pi khởi động lại
...
- [x] Bao độ cao dưới sàn: drone nằm bàn (0,17 m), "trái 0,5" → Δ`CLP` = 0; "xuống 0,3" → `OB_T_UP = 0`,
      `CLP` tăng; "xuống 0,005" → Δ`CLP` = 0 — *Pi đo 09-13, đạt* — **lịch sử: sàn đã bỏ 09-13**
- [ ] Xuống chậm sát đất (firmware sau 09-13): *nằm bàn — Pi đo 09-14, đạt; còn ca kê cao > 1,2 m* — "xuống 0,3" → `OB_T_UP = −0,300`, Δ`CLP` = 0;
      "xuống 0,5" → `−0,300`, `CLP` tăng; kê cao > 1,2 m laser, "xuống 0,8" → `−0,800` — *FC đã thử
      trên target qua SWD, chờ Pi đo qua MAVROS*
...
**Điều kiện bắt đầu:** *(Pi đo 09-14: thoả)* FC chạy bản sạch có trong lịch sử phiên bản — hiện là **1.5, commit `df169c5`** (hash `df169c5c0ae19ac5`) — *trước đó 1.4, commit `6b75e72`,
...
- [ ] Pi arm trên mặt đất (cánh lắp, người đứng xa, tay trên ch5): động cơ lên ~50 % ga và máy bay **nằm
      yên**, không nhấc khỏi đất cho tới khi Pi lệnh leo — dựa trên ga treo đo 0,65 (6.2)
