...
| `DRONE_TELEMETRY` | ~111 B | 2 Hz | 222 B/s |
...
| Hạng mục | Giá trị |
|---|---|
| Trạng thái | **[THOẢ THUẬN]** — Pi **đã hiện thực và kiểm ba tình huống** (10.B); GCS đã có sẵn đường (khoá 32 byte, `MAVLink_signing`, đếm `rx_bad_sig`) nhưng chưa nối. **GCS mặc định BẬT còn Pi mặc định TẮT** — xem cảnh báo ngay dưới bảng |
...
> **Cảnh báo bất đối xứng mặc định — đọc trước khi bật.** GCS mặc định **BẬT** chữ ký, Pi mặc định
> **TẮT**. Hai bên lệch nhau ở đây thì bên bật sẽ **bỏ sạch** gói của bên kia, và triệu chứng
...
| **42005** | `DRONE_TAGMAP_COUNT` | GCS→Pi | **[ĐỀ XUẤT]** — GCS nêu, chờ Pi (11.6 P31) |
| **42006** | `DRONE_TAGMAP_REQUEST` | Pi→GCS | **[ĐỀ XUẤT]** |
| **42007** | `DRONE_TAGMAP_ITEM` | GCS→Pi | **[ĐỀ XUẤT]** |
| **42008** | `DRONE_TAGMAP_ACK` | Pi→GCS | **[ĐỀ XUẤT]** |

**Còn trống:** `42009`, `42012–42099`. Chưa cấp cho ai.
...
> thái 9 chỉ sống **đúng một tick 200 ms**. Từ 0.5 Pi **giữ nó 1,5 s** (ba gói telemetry), nên nó lên
> dây được.
>
> **Nhưng đừng đọc giá trị 9 là "nhiệm vụ thành công".** Nó là đích chung của **cả** huỷ lệnh, RTH,
> `NAV_LAND`, hết lượt thử **và** hết kế hoạch — nghĩa đúng của nó là *"chuyến bay đã kết thúc sạch
> sẽ, drone đã đậu và tắt động cơ"*. Muốn biết **vì sao** kết thúc thì đọc `flight_result` (8.5b).
...
### 8.5b `flight_result` — vì sao chuyến bay kết thúc [MỚI Ở 0.5, Pi đề xuất]

`DRONE_TELEMETRY.flight_result`, `uint8`, enum `DRONE_FLIGHT_RESULT`, **trường extension** (sau
`<extensions/>`, nên `CRC_EXTRA` của 42010 vẫn là **14** — Pi đã sinh mã từ cả hai bản XML và đối
chiếu: cả sáu `CRC_EXTRA` từ 42001 đến 42011 giữ nguyên).

| Giá trị | Tên | Nghĩa |
|---|---|---|
| 0 | `UNKNOWN` | Chưa chuyến nào kết thúc kể từ khi Pi chạy, **hoặc** gói đến từ Pi cũ chưa khai trường này (R3b) |
| 1 | `COMPLETED` | Làm hết mọi điểm của kế hoạch, không có gì cắt ngang |
| 2 | `ABORTED` | GCS huỷ nhiệm vụ (`MAV_CMD_DRONE_ABORT_MISSION`) |
| 3 | `RTH` | Về nhà rồi hạ cánh |
| 4 | `LANDED_CMD` | Hạ cánh tại chỗ theo `NAV_LAND` |
| 5 | `FAILSAFE` | Sự cố, hoặc Pi mất quyền điều khiển |
| 6 | `RETRIES_EXHAUSTED` | Hết lượt thử tìm/hạ theo tag — hạ cánh tại chỗ. **Chưa làm được việc**, dù vẫn đi qua `MISSION_COMPLETE` |

Ba tính chất GCS cần biết:

1. **CHỐT LẠI.** Giá trị giữ nguyên sau khi FSM về `IDLE`, cho tới lúc **cất cánh** chuyến mới (đúng
   lúc `IDLE → TAKEOFF`, không phải lúc nạp kế hoạch hay lúc nhận `MISSION_START` — yêu cầu cất cánh
   có thể bị huỷ trước khi rời đất, và khi đó kết quả chuyến trước vẫn còn giá trị). Nhờ vậy GCS khởi
   động lại **bao lâu sau** vẫn hỏi lại được, thoả R5 — đây là điều mà mình việc giữ trạng thái 9 lâu
   hơn **không** làm được, vì nó chỉ cứu được GCS đang nghe trong đúng cửa sổ 1,5 s.
2. **Lý do ĐẦU TIÊN thắng.** Đang hạ cánh vì failsafe mà GCS bấm huỷ thì kết quả vẫn là `FAILSAFE`,
   không bị ghi đè thành `ABORTED`: ghi đè sẽ giấu mất sự cố.
3. **`0` không phải "chưa xong".** Theo R3, `0` nghĩa là *không biết*. GCS đừng hiển thị gì khi đọc 0.
...
**`DRONE_TAGMAP_ITEM`** (42007, GCS→Pi):
...
| ~~12~~ | ~~P30: giữ `MISSION_COMPLETE` + `flight_result`~~ | **XONG ở 0.5** (11.5). `MISSION_COMPLETE_HOLD_S = 1.5`; `DRONE_TELEMETRY.flight_result` chốt lại tới lần cất cánh sau; sửa kèm lỗi kẹt FSM khi huỷ đến giữa cửa sổ giữ. 8 pytest mới |

Còn lại để lên **1.0**: việc **8** ở trên, `TIMESYNC` (7.4) cho `rtt_ms`, nối chữ ký hai đầu (7.6),
và bốn phép kiểm 10.A chưa chạy (A11, A15b, A16, A17).
...
Hợp đồng giữ **0.x** cho tới lúc đó. **Nhưng từ 10.D thì lý do đã khác:** trước đây là "chưa byte nào
chạy trên dây nên chưa có ai để mà phá tương thích" — câu đó **hết hiệu lực từ 2026-09-16**. Nay đã có
hai bản đang chạy thật, nên **kể cả ở 0.x, đổi `CRC_EXTRA` vẫn là phá tương thích thật**, và phải theo
đúng 6.1: trường mới thì đặt **sau `<extensions/>`**. Bản 0.5 làm đúng vậy và đã đối chiếu cả sáu
`CRC_EXTRA` trước/sau — không cái nào đổi.


### 11.5 Mục GCS nêu sau lần chạy thật đầu tiên — Pi đã phân giải ở 0.5

**P30 · Cao — FSM không bao giờ đi qua `MISSION_COMPLETE`, nên "nhiệm vụ đã xong" không có dấu hiệu nào
trên dây.** [ĐÃ SỬA Ở 0.5 — xem "Pi trả lời" cuối mục]
...
#### Pi trả lời (0.5): làm (a), **1,5 giây**, và thêm một trường — vì (a) một mình chưa đủ

**Chẩn đoán của GCS đúng, và Pi tái hiện được nguyên nhân:** vòng FSM chạy 5 Hz (200 ms/tick) còn
telemetry phát 2 Hz (500 ms), nên `MISSION_COMPLETE` sống **đúng một tick** rồi về `IDLE` ngay chu kỳ
sau. Xác suất một gói telemetry rơi trúng ≈ 200/500 = 40%; trượt cả hai chuyến là chuyện bình thường.

**Làm (a):** `MISSION_COMPLETE_HOLD_S = 1.5` trong `mission_fsm.py` — ba chu kỳ telemetry, còn đúng
ngay cả khi mất hai gói liên tiếp. Giữ 0,5 s đúng bằng một chu kỳ là **thế dao**: chỉ cần jitter là
trượt. Trong lúc giữ, drone đã đậu và đã disarm nên không lệnh nào được phát ra.

**Nhưng (a) một mình vẫn để hở đúng cái lỗ mà GCS nêu.** GCS viết rằng (a) "giữ được R5: một GCS vừa
khởi động lại vẫn đọc được chuyến trước đã xong" — điều đó **chỉ đúng nếu GCS khởi động lại kịp trong
cửa sổ 1,5 s**. Khởi động lại ở giây thứ mười thì vẫn mù y như cũ. R5 đòi trạng thái **hỏi lại được**,
mà một trạng thái thoáng qua thì về bản chất không hỏi lại được, dù giữ bao lâu.

**Và có một lỗi thứ hai mà (a) sẽ biến thành lỗi nặng hơn hiện tại.** Trong FSM của Pi,
`MISSION_COMPLETE` là đích chung của **cả** huỷ lệnh, RTH, `NAV_LAND`, hết lượt thử **và** hết kế
hoạch. Đây không phải chuyện tình cờ — Pi có sẵn một test tên
`test_abort_xong_thi_bao_MISSION_COMPLETE_chu_khong_FAILSAFE` khẳng định rằng **lệnh huỷ hoàn thành
đúng thì phải hiện `MISSION_COMPLETE`**, để GCS không hiện FAILSAFE oan cho một lệnh chạy trót lọt.
Nếu Pi chỉ làm (a) và GCS đọc giá trị 9 là "nhiệm vụ hoàn thành", thì **mọi lệnh huỷ sẽ được báo là
thành công**. Hiện tại GCS ít nhất còn *biết là mình không biết*; (a) trần sẽ biến nó thành *tin chắc
vào điều sai*. Đấy là bước lùi.

Riêng trường hợp **hết lượt thử tìm tag** còn tệ hơn: drone chưa bao giờ thấy tag, hạ xuống chỗ lạ, và
vẫn kết thúc ở `MISSION_COMPLETE` y hệt chuyến thành công.

**Nên Pi làm (a) + `DRONE_TELEMETRY.flight_result`** (8.5b): `uint8`, **extension** nên `CRC_EXTRA` của
42010 không đổi (vẫn 14, Pi đã sinh mã từ cả hai bản XML và đối chiếu cả sáu bản tin), `0` = không biết
theo R3b, và **chốt lại cho tới lần cất cánh sau** nên GCS khởi động lại lúc nào cũng hỏi được. Đây mới
là thứ thoả R5 thật. Chi phí phía GCS: đọc thêm một byte; nhánh xử lý trạng thái 9 mà GCS đã giữ sẵn
vẫn dùng được nguyên.

**Một lỗi kẹt mà việc giữ trạng thái phơi ra, Pi sửa luôn:** `TRANSITIONS[MISSION_COMPLETE]` chỉ có
`IDLE`, **không có nhánh `FAILSAFE`**. Nhánh xử lý huỷ nhiệm vụ lại trả về sớm mỗi tick khi không tìm
được đường đi, nên nếu lệnh huỷ đến **trong** cửa sổ giữ 1,5 s thì FSM **kẹt ở `MISSION_COMPLETE` vĩnh
viễn**. Trước 0.5 cửa sổ đó chỉ rộng 200 ms nên gần như không ai gặp; (a) làm nó rộng gấp bảy lần rưỡi.
Đã thêm đường ra riêng và test `test_abort_giua_luc_giu_MISSION_COMPLETE_khong_lam_fsm_ket`.
...
### 11.6 Mục GCS nêu ở 0.5 — Pi đã trả lời, chờ GCS sửa 8.7

**P31 · Trung bình — nạp bản đồ tag qua dây, thay cho việc chép `tags.yaml` bằng tay.** [ĐỀ XUẤT]

Mục 8.6 đã để ngỏ việc này (*"Nạp bản đồ qua dây để MINOR sau nếu cần"*). GCS đề nghị làm bây giờ,
và đây là lý do cụ thể chứ không phải cho tiện.

**Vì sao cần.** Quy trình hiện tại là: sửa bản đồ ở GCS → tải `tags.yaml` → chép sang Pi → khởi động
lại Pi. Bốn bước, ba trong số đó nằm ngoài phần mềm và **không ai kiểm được là đã làm đúng chưa** cho
tới khi `tagmap_crc` khớp. Người vận hành sửa một tag rồi quên chép, hoặc chép nhầm file cũ, thì GCS
khoá nạp kế hoạch và họ đi tìm nguyên nhân ở chỗ khác. Đây không phải chuyện giả định: trong lúc thử
2026-09-16, GCS xuất ra một `tags.yaml` **sai định dạng** (danh sách lồng thay vì danh sách phẳng,
thiếu header `ros__parameters`, trộn `int` với `float`) và chỉ lộ ra khi đối chiếu bằng tay với bản
của Pi — chép sang là ROS từ chối nạp. Một đường truyền có `ACK` sẽ bắt được việc đó ngay.

**Vì sao KHÔNG phải vì GCS muốn giành quyền định vị.** Bản đồ tag vẫn do người vận hành soạn ở GCS
và Pi vẫn là bên duy nhất quyết định kế hoạch có hợp lệ không (1.1). Cái đổi chỉ là **đường đi của
file**, không phải ai sở hữu nó.

**Đề xuất cụ thể:** bốn bản tin `42005–42008` ở **mục 8.7**, giữ nguyên hình dạng bắt tay của mục 3
(count → request → item → ack) vì cơ chế đó đã chạy đúng trên dây ngày 16/09, và vì Pi đã có sẵn mã
xử lý cùng kiểu — thêm một lượt nạp thứ hai rẻ hơn nhiều so với nghĩ ra cơ chế mới.

**Ba chỗ GCS không tự quyết được, cần Pi trả lời:**

1. **Sau khi nhận đủ, Pi làm gì?** Ghi đè `config/tags.yaml` rồi nạp lại tham số của ba node đang
   đọc nó (`marker_pose_republisher`, `landing_target_bridge`, `mission_manager`), hay ghi file rồi
   yêu cầu người vận hành khởi động lại? Nếu là cách sau thì `tagmap_crc` chưa đổi ngay sau `ACCEPTED`,
   và quy tắc 2 ở mục 8.7 phải viết lại — GCS cần biết để không báo "đồng bộ xong" khi chưa xong.
2. **Tag mới có cần khai trong `apriltag.yaml` không?** Nếu bộ phát hiện chỉ publish TF cho các tag
   liệt kê ở `tag.ids`/`tag.frames` thì nạp bản đồ qua dây **không đủ** để thêm một tag chưa từng khai,
   và GCS phải nói rõ giới hạn đó cho người vận hành thay vì để họ tưởng đã xong.
3. **Giới hạn 32 tag** GCS đặt theo cảm tính (gấp đôi giới hạn 16 mục kế hoạch). Pi thấy con số nào
   hợp lý hơn thì sửa.

**Phía GCS chưa viết code cho P31** — theo 6.4, cách điền từng trường phải chốt trước khi viết, và
mục 0.3 ghi [ĐỀ XUẤT] thì chưa được dựa vào để viết code. Số hiệu ở 8.1 đang giữ chỗ, đổi được.

**Kèm theo, để Pi biết GCS KHÔNG định làm gì:** GCS sắp cho người vận hành chỉ định "drone đang đậu
ở đâu" khi `POS_VALID = 0`, nhưng con số đó **chỉ để vẽ trong cảnh 3D của GCS và không bao giờ lên
dây**. GCS cố ý không xin một đường ép pose xuống EKF, vì `home` của RTH được chốt ở tick đầu tiên
có `POS_VALID` trong lúc TAKEOFF (5.2b ý 4): ép pose lúc drone còn trên đất sẽ làm `home` chốt theo
toạ độ gõ tay, tức tái tạo đúng lỗi P19 mà Pi vừa sửa xong, chỉ khác nguồn gây ra.

#### Pi trả lời (0.5): nhận hướng đi, nhưng 8.7 phải sửa ba chỗ trước khi chốt

Pi đọc lại mã của chính mình trước khi trả lời, và tìm ra một chỗ đề xuất dựa trên thông tin sai: `tags.yaml`
được **bốn** node trên drone thật đọc, không phải ba. Node bị sót là **`telemetry_aggregator_node`** — chính
node tính `tagmap_crc` — tức đúng node mà quy tắc 2 của 8.7 dựa vào. (Trong mô phỏng còn `sim_tag_node`.)

**Câu 1 — Pi ghi file và cần khởi động lại stack. Chưa nạp lại nóng được, và 0.5 không làm.**
...
**Hệ quả: quy tắc 2 viết lại như sau.** `ACCEPTED` nghĩa là *"Pi đã kiểm và ghi bản đồ; có hiệu lực sau
khi stack khởi động lại"* — **không** phải "đã có hiệu lực". GCS xác nhận hiệu lực bằng **một cách duy
nhất**: `DRONE_TELEMETRY.tagmap_crc` bằng đúng `tagmap_crc` GCS đã khai trong `DRONE_TAGMAP_COUNT`. Giữa
hai mốc, GCS hiện "đã gửi, chờ khởi động lại" và vẫn khoá nạp kế hoạch như 8.6. Quy tắc này **vẫn đúng
nếu sau này Pi làm nạp nóng** — khi đó hai mốc chỉ trùng nhau — nên GCS viết code theo nó thì không phải
sửa lại. Không cần thêm trường.

**Câu 2 — Có, phải khai trong `apriltag.yaml`. Và vì thế `frame_name` phải bỏ.**

Trong `apriltag_ros` (`AprilTagNode.cpp`), khi `tag.frames` không rỗng thì detection của tag không có trong
danh sách **bị bỏ qua hoàn toàn**, không phát TF; còn `tag.ids` / `tag.frames` / `tag.sizes` khai
`read_only`, đổi phải khởi động lại node. Pi đối chiếu trên mã nguồn gốc (nhánh `master`); máy phát triển
chưa cài gói nên **chưa kiểm trên bản apt Jazzy**. Thêm nữa, kích thước tag nằm ở `tag.sizes` mà 8.7 không
mang — sai size là sai khoảng cách đúng bằng chừng ấy phần trăm. Nên:

1. **Nạp qua dây chỉ đổi được toạ độ của tag ĐÃ KHAI** trong `apriltag.yaml`. Thêm tag mới vẫn phải sửa
   `apriltag.yaml` trên Pi bằng tay. GCS nói rõ giới hạn này với người vận hành.
2. **Bỏ `frame_name` khỏi `DRONE_TAGMAP_ITEM`.** Tên khung phải trùng `tag.frames` của `apriltag.yaml`, tức
   Pi đã có sẵn và đó là nguồn duy nhất. Để GCS gửi thì chỉ thêm một chỗ có thể lệch — và `frame_name`
   **không nằm trong CRC 8.6**, nên lệch ở đó thì CRC vẫn khớp và không ai thấy. "Rỗng = Pi tự đặt tên"
   cũng không làm được, vì cùng lý do.
3. **Thêm mã `6 ERR_UNDECLARED_TAG`** vào `DRONE_TAGMAP_ACK`: có `tag_id` không nằm trong `tag.ids` của
   `apriltag.yaml` → Pi từ chối **cả lượt**, `reason` ghi id đầu tiên bị thiếu. Không nạp một phần.

**Câu 3 — 32 được, giữ nguyên.** Phía Pi không có giới hạn nào gần con số đó.

**Pi nhận nguyên:** số hiệu `42005–42008` (còn trống trong `drone_gcs.xml`); quy tắc 1 — FSM vốn đã chỉ
nhận kế hoạch khi `IDLE`, và cũng như 11.5, trong 1,5 s giữ `MISSION_COMPLETE` thì Pi trả `ERR_BUSY`;
quy tắc 3 — Pi xoá kế hoạch **ngay khi `ACCEPTED`**, không đợi khởi động lại, để không ai `MISSION_START`
được trong khoảng giữa. Pi cũng đồng ý với ghi chú cuối của GCS: không có đường ép pose xuống EKF.

**Việc tiếp theo:** GCS sửa 8.7 theo ba điểm trên (quy tắc 2, bỏ `frame_name`, thêm mã 6) → Pi thêm bốn
bản tin vào `drone_gcs.xml`, tăng số, rồi mới viết code. **Pi chưa viết code cho P31**, theo đúng 6.4.
...
| Nạp bản đồ tag qua dây | 8.7, 11.6 |
